...
nlohmann::json readInfo(const std::string &path) {
	...
	if(in.contains("info")) {
		if(in["info"].contains("title"))	out["title"] = in["info"]["title"];
		if(in["info"].contains("author"))	out["author"] = in["info"]["author"];
		if(in["info"].contains("version"))	out["version"] = in["info"]["version"];
		if(in["info"].contains("revision"))	out["revision"] = in["info"]["revision"];
